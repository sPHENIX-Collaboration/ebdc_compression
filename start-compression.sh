...
# find ${folder} -name "*.evt" |  parallel --joblog ${jobname}/parallel.log --progress -j${job} "cat {} | pv -btrnf -i 10000  2>>${jobname}/pv_in_{#}.log  | ${zipcmd} -${ziplevel} -c | pv -btrnf -i 10000  2>>${jobname}/pv_out_{#}.log  > /dev/null "
